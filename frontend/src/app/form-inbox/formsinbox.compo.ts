...
@Component({
  selector: 'app-formsinbox',
  templateUrl: './formsinbox.compo.html'
})

export class FormsInboxComponent implements OnInit {
  ...
  loadView(forceUpdate: boolean): void {
    ...
    this.backendSVC.actionContent('getAllSubmittedForms', {}).then(
      (data: any) => {
        if (data.success) {
