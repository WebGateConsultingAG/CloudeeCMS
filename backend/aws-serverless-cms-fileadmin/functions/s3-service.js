 ...
 * 
 * File Version: 2020-10-21 14:48 - RSC
 */
...
s3service.batchDelete = function (bucketname, lstKeys, done) {
  console.log("delete objects from s3", bucketname, lstKeys);
  var keys = [];
  for (let k in lstKeys) {
    keys.push({ Key: lstKeys[k] });
  }
  var params = {
    Bucket: bucketname,
    Delete: {
  ...
  s3.deleteObjects(params, function (err, data) {
    if (err) {
      ...
      done.done({ success: true, msg: data });
    }
  });
};
