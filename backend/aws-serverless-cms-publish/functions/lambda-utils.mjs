...
const DDBBatchGet = async (params) => {
    // this will get beyond 1MB limit
    let lstRC = [];
    let hasMore = true;
    delete params.ExclusiveStartKey; // make sure there is no lastevaluated key from a previous run
    while (hasMore) {
        let data = await documentClient.batchGet(params);
        for (let lstR in data.Responses) {
            data.Responses[lstR].forEach((itemdata) => {
                lstRC.push(itemdata);
            });
        }
...
export { documentClient, lambdaCB, getFormattedDate, getNewGUID, DDBGet, DDBQuery, DDBBatchGet, DDBScan };
